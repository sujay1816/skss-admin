...
// Converts product name to URL-safe slug
...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  const uploadVideo = async (file: File) => {
    ...
    try {
      ...
      if (data.secure_url) { setVideoUrl(data.secure_url); toast.success('Video uploaded!') }
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        const { data: newImg } = await supabase.from('product_images').insert({
          product_id: params.id, url: data.secure_url, public_id: data.public_id,
          is_primary: isPrimary, order_index: images.length, alt_text: form?.name || '',
        }).select().single()
  ...
  const deleteImage = async (imgId: string) => {
    ...
    if (remaining.length > 0) {
      ...
      if (wasPrimary) {
        ...
        setImages(remaining.map((i, idx) => ({ ...i, is_primary: idx === 0 })))
      }
    }
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('products').update({
      name: form.name,
      slug: newSlug,           // ← slug kept in sync with name
    ...
    // Update slug in form state so further saves use new slug
    setForm((p: any) => ({ ...p, slug: newSlug }))
    ...
    toast.success('Product updated! Name and slug updated.')
    setSaving(false)
  }
    ...
    <AdminLayout>
      <div className="max-w-3xl">
          ...
          <div>
            ...
            {form.slug && (
              <p className="text-xs text-gray-400 mt-0.5">
                ...
                {slugify(form.name) !== form.slug && (
                  <span className="ml-2 text-orange-500">→ will update to: {slugify(form.name)}</span>
                )}
              </p>
            )}
          </div>
        ...
        <div className="card p-5 space-y-4 mb-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <F label="Original Price (₹)">
              <input className="input" type="number" value={form.original_price || ''} onChange={e => setForm((p: any) => ({ ...p, original_price: e.target.value }))} />
            </F>
            <F label="Sale Price (₹)">
              <input className="input" type="number" value={form.sale_price || ''} onChange={e => setForm((p: any) => ({ ...p, sale_price: e.target.value }))} placeholder="Leave empty for no sale" />
            </F>
            <div className="col-span-2">
              <F label="Description">
                <textarea className="input" style={{ height: 100, resize: 'none' }} value={form.description || ''} onChange={e => setForm((p: any) => ({ ...p, description: e.target.value }))} />
              </F>
            </div>
            <F label="Care Instructions">
              <input className="input" value={form.care_instructions || ''} onChange={e => setForm((p: any) => ({ ...p, care_instructions: e.target.value }))} />
            </F>
            <div className="flex flex-wrap gap-4 col-span-2 pt-2">
              {[['is_active','Published'],['is_featured','Featured'],['is_bestseller','Bestseller'],['blouse_included','Blouse Included']].map(([k, l]) => (
                <label key={k} className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={!!form[k]} onChange={e => setForm((p: any) => ({ ...p, [k]: e.target.checked }))} className="w-4 h-4" style={{ accentColor: 'var(--crimson)' }} />
                  {l}
                </label>
              ))}
            </div>
        ...
        <div className="card p-5 mb-5">
              ...
              {images.map(img => (
                <div key={img.id} className="relative group">
                  <div className="relative aspect-square border-2 rounded-lg overflow-hidden"
                    ...
                    {img.is_primary && (
                      <div className="absolute top-0 left-0 right-0 text-center py-0.5 text-white font-medium"
          ...
          <p className="text-xs text-gray-400 mt-3">Hover over an image to set as primary or delete. Primary image shows first in the storefront.</p>
        </div>
        ...
        <div className="card p-5 mb-5">
          ...
          <p className="text-xs text-gray-400 mb-4">Short saree drape video (15-60 sec). MP4 format, max 50MB.</p>
          ...
          {videoUrl ? (
            ...
            <button type="button" onClick={() => videoRef.current?.click()} disabled={uploadingVideo}
              ...
              {uploadingVideo
                ...
                : <><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2"><polygon points="5 3 19 12 5 21 5 3"/></svg><p className="text-xs text-gray-400">Click to upload product video (MP4)</p></>
              }
            </button>
          )}
        </div>
