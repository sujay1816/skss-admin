...
const STATUSES = ['placed','confirmed','shipped','delivered','cancelled','return_approved','return_rejected','refunded']
const STATUS_COLORS: Record<string, string> = { placed:'#6B7280',confirmed:'#059669',shipped:'#D97706',delivered:'#16A34A',cancelled:'#DC2626',return_requested:'#7C3AED',return_approved:'#1565C0',return_rejected:'#DC2626',refunded:'#059669' }
...
export default function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const save = async () => {
    setSaving(true)
    const { error } = await supabase.from('orders').update({ status, tracking_id: trackingId || null, courier_name: courierName || null, updated_at: new Date().toISOString() }).eq('id', params.id)
    if (!error) {
      toast.success('Order updated!')
      // Send WhatsApp if shipped and not already sent
      if (status === 'shipped' && trackingId && order.profiles?.whatsapp_opted_in && !waNotified) {
        try {
          await fetch('/api/whatsapp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ orderId: params.id, phone: order.profiles.phone, orderNumber: order.order_number, trackingId, courierName }) })
    ...
    <AdminLayout>
      <div className="max-w-4xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="badge text-white" style={{ background: STATUS_COLORS[order.status] || '#6B7280', fontSize: 13, padding: '4px 14px' }}>{order.status.replace('_',' ')}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2"><label className="text-xs text-gray-600 mb-1 block">Order Status</label>
                  <select className="input" value={status} onChange={e => setStatus(e.target.value)}>
                    {STATUSES.map(s => <option key={s} value={s} className="capitalize">{s.replace('_',' ')}</option>)}
                  </select>
                </div>
                <div><label className="text-xs text-gray-600 mb-1 block">Courier Name</label><input className="input" value={courierName} onChange={e => setCourierName(e.target.value)} placeholder="Delhivery, BlueDart..." /></div>
                <div><label className="text-xs text-gray-600 mb-1 block">Tracking ID</label><input className="input" value={trackingId} onChange={e => setTrackingId(e.target.value)} placeholder="AWB number" /></div>
              </div>
              {status === 'shipped' && trackingId && !waNotified && order.profiles?.whatsapp_opted_in && (
                <p className="text-xs mt-3 p-2 bg-green-50 border border-green-200 rounded text-green-700">✓ WhatsApp notification will be sent to {order.profiles?.phone} when you save.</p>
              )}
              {waNotified && <p className="text-xs mt-3 text-green-600">✓ WhatsApp notification already sent</p>}
              <button onClick={save} disabled={saving} className="btn btn-primary mt-4">{saving ? 'Saving...' : 'Update Order'}</button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="card p-4">
              ...
              {order.profiles?.whatsapp_opted_in && <span className="text-xs text-green-600 mt-1 block">✓ WhatsApp opted in</span>}
            </div>
            <div className="card p-4">
              <h2 className="font-semibold text-gray-900 mb-3">Delivery Address</h2>
              <p className="text-sm font-medium">{order.address_snapshot?.fullName}</p>
              <p className="text-xs text-gray-500">{order.address_snapshot?.addressLine1}</p>
              {order.address_snapshot?.addressLine2 && <p className="text-xs text-gray-500">{order.address_snapshot.addressLine2}</p>}
              <p className="text-xs text-gray-500">{order.address_snapshot?.city}, {order.address_snapshot?.state}</p>
              <p className="text-xs text-gray-500">{order.address_snapshot?.pincode}</p>
              <p className="text-xs text-gray-600 mt-1 font-medium">📞 {order.address_snapshot?.phone}</p>
            </div>
            <div className="card p-4">
              ...
              <div className="space-y-1.5 text-sm">
                ...
                {order.coupon_discount > 0 && <div className="flex justify-between"><span className="text-green-600">Discount</span><span className="text-green-600">−{formatPrice2(order.coupon_discount)}</span></div>}
                ...
                <div className="flex justify-between font-semibold border-t pt-2" style={{ borderColor: '#F3F4F6' }}><span>Total</span><span style={{ color: 'var(--crimson)' }}>{formatPrice2(order.total_amount)}</span></div>
                ...
                <div className="flex justify-between text-xs"><span className="text-gray-500">Payment</span><span className={`font-medium ${order.payment_status === 'paid' ? 'text-green-600' : 'text-orange-500'}`}>{order.payment_status}</span></div>
              </div>
            </div>
            {order.return_reason && (
              <div className="card p-4 border-l-4" style={{ borderLeftColor: '#7C3AED' }}>
                ...
                <p className="text-xs text-gray-400 mt-1">{order.return_requested_at && new Date(order.return_requested_at).toLocaleDateString('en-IN')}</p>
              </div>
            )}
          </div>
        </div>
